  ...
  new Strategy(
    ...
    async (username, password, done) => {
      try {
        ...
        if (!checkUser) throw new Error('Invalid login Credentials');
        ...
        const { _id, full_name, profile_photo, genotype, blood_group } =
          checkUser;

        done(null, { _id, full_name, profile_photo, genotype, blood_group });
